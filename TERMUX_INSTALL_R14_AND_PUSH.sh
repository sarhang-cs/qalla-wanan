...
BACKUP="$HOME/QALLA-WANAN-backup-before-R14-$STAMP"
GIT_RESTORE="$HOME/.qalla-r14-git-restore"
ENV_KEEP="$HOME/.qalla-r14-env-local-$STAMP"
FONT_KEEP="$HOME/.qalla-r14-money-heist-font-$STAMP.ttf"
FONT_NAME="UniQAIDAR-Money-Heist-002.ttf"

printf '\n=================================================\n'
printf ' QALLA WANAN R14 — MONEY HEIST MAP FONT + RTL\n'
printf '=================================================\n'
...
for required in package.json src/nav-map.js scripts/check-data.mjs public/data/nav/labels-native.geojson; do
  [ -s "$SOURCE/$required" ] || { echo "❌ پەکەجی R14 ناتەواوە: $required"; exit 1; }
done

# The font is intentionally not redistributed inside the ZIP. It is copied from
# the user's own Downloads folder (or preserved from an earlier installation).
FONT_SOURCE=""
for candidate in \
...
echo "✅ فۆنت دۆزرایەوە: $FONT_SOURCE ($FONT_BYTES bytes)"
...
tar -C "$SOURCE" --exclude='.git' --exclude='node_modules' --exclude='dist' --exclude='public/fonts/UniQAIDAR-Money-Heist-002.ttf' -cf - . | tar -C "$TARGET" -xf -
...
printf '📦 فۆنتی ناوی ماپ، RTL، 69,000 شوێن، Satellite و GPS دەپشکنرێن...\n'
...
NODE
 ...
 grep -q "UniQAIDAR Money Heist 002" src/nav-map.js
 grep -q "UniQAIDAR-Money-Heist-002.ttf" src/nav-map.css
 grep -q 'const width = 46' src/nav-map.js
 grep -q "text-halo-width': 0" src/nav-map.js
 ! grep -q 'MAP_LABEL_FONT.*Vazirmatn' src/nav-map.js
...
if ! git diff --cached --quiet; then
  git commit -m "R14: fix Kurdish Arabic RTL shaping, install Money Heist map font, rebalance native labels"
else
...
printf '\n=================================================\n'
printf '✅ R14 Push کرا\n'
printf '✅ فۆنتی ماپ لە Downloads گوازرایەوە و Build کرا\n'
printf '✅ فۆنتی UI ـی ئەپ نەگۆڕدرا\n'
printf '✅ RTL کوردی/عەرەبی و قەبارەی ناوەکان ڕێکخرا\n'
