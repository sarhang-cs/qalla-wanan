...
const installer = fs.readFileSync('TERMUX_INSTALL_R14_AND_PUSH.sh', 'utf8');

const required = [
  ['R14 release', /2026-07-23-qalla-wanan-r14-money-heist-rtl/],
  ['RTL plugin', /setRTLTextPlugin\(RTL_PLUGIN_URL,\s*false\)/],
  ['map font constant', /const MAP_LABEL_FONT = 'UniQAIDAR Money Heist 002'/],
  ...
  ['font-face CSS', /@font-face\{font-family:"UniQAIDAR Money Heist 002"/],
  ...
  ['compact capsule geometry', /const width = 46;[\s\S]*const height = 20;/],
  ['balanced detail POIs', /tier: 'poi_detail'.*minzoom: 18\.0/],
...
const forbidden = [
  ...
  ['map font applied to app UI', /--app-font:"UniQAIDAR Money Heist 002"/]
];
const failures=[];
...
if (manifest.release !== '2026-07-23-qalla-wanan-r14-money-heist-rtl') failures.push('manifest release mismatch');
...
console.log(JSON.stringify({
  ok:true,
  contract:'R14-MONEY-HEIST-RTL-SATELLITE-GPS-69000',
  renderer:'MapLibre native WebGL symbol layers',
  mapLabelFont:'UniQAIDAR Money Heist 002',
